...
class Saisho {
	...
	public function renderPageList(array $list) {
        if (empty($list)) return '<h1>ʅ(°⊱,°)ʃ Nothing found</h1>';
	...
	private function pageExists(string $pageName) {
		$list = $this->listPages();
		return in_array($pageName, $list);
	}

	private function getRawContent(string $pageName) {
		return file_get_contents($pageName);
	}
	...
	private function parseYaml(string & $str) {
		...
		if (!isset($yaml[0])) {
			return (object)['body' => $str]; /* Return teh rest of the content if header missing */
		}
...
header('x-generator: saisho mk.2');
...
<head>
  ...
  <title><?php echo isset($page->content->title) ? $page->content->title . ' &mdash; ' . META_TITLE : META_TITLE; ?></title>
  <style>
...
$saisho->saveFeed();
echo '<pre>' . round(((microtime(true) - $time) * 1000), 3) . 'ms</pre>';